...
export class ToolProvider {
  ...
  constructor(public vm: any) {
    ...
    if (ratioX !== undefined) {
      ...
      ratioN = width.indexOf('%') < 0 && height.indexOf('%') < 0 ? round5th(parseInt(width) / parseInt(height)) : n169;
    }
  ...
  generateDiv(): string {
    ...
    if (this.vm.selWidth() === 'pixel') {
      ...
      if (!$.isNumeric(pixel)) {
        ...
        if (this.vm.optProvider() === 'bandcamp') {
          ...
          if (nWidth < minWidth) {
            this.vm.errIframe(
              'Bandcamp erfordert eine Mindestbreite von 170 Pixel - bei Layout=standard und Artwork=small sogar 250 Pixel!'
            );
          }
          if (nWidth > 700) {
    ...
    if (this.vm.txtElementID().length === 0) {
      ...
      this.vm.errIframe(`Bitte geben Sie eine gültige ${id.slice(0, id.length - 1)} ein!`);
    }
    // assemble div clause
    return `&lt;div class="html5video ${this.vm.optProvider()}${
      this.params
    }" id="${this.vm.txtElementID()}"&gt;&lt;/div&gt;`;
  }
  ...
  validateColor(property: string, description: string, param: string, standard: string): void {
    ...
    if (value.length > 0) {
      if (!isValidColor(value)) {
        this.vm.errIframe(
          `Bitte geben Sie einen gültigen Wert (hexadezimal, 3- oder 6-stellig) für die ${description}farbe ein!`
        );
      } else {
