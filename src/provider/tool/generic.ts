...
export class ToolProvider {
    ...
    constructor(public vm: any) {
        ...
        let ratioX: string = vm.$tag.attr('data-ratio');
        let ratioN: number;
        ...
        if (ratioX!==undefined) {
            ...
            let width = vm.$tag.attr('width') || '%';
            // extract height if available
            let height = vm.$tag.attr('height') || '%';
        ...
        if (this.vm.selWidth()==='pixel') {
            ...
            if (!$.isNumeric(pixel)) {
                ...
                if (this.vm.optProvider()==='bandcamp') {
                    let minWidth = (this.vm.optLayout()==='standard' && this.vm.optArtwork()==='small' ? 250 : 170);
                    let nWidth = parseInt(pixel);
                    if (nWidth<minWidth) {
    ...
    validateNumber(property: string, description: string, param: string): void {
        ...
        let value: string = this.vm[property]();
        if (value.length>0) {
            if (!$.isNumeric(value)) {
